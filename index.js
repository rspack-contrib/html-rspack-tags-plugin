...
function HtmlRspackTagsPlugin (options) {
...
HtmlRspackTagsPlugin.prototype.apply = function (compiler) {
  ...
  const onCompilation = compilation => {
    ...
    const onAlterAssetTagGroups = (htmlPluginData, callback) => {
          ...
          if (isDefined(tag.path)) {
            return {
              ...
              attributes: {
                ...
                ...Object.fromEntries(Object.entries(tag.attributes).map(([key, value]) => (typeof value === 'string' || typeof value === 'boolean') ? [key, value] : [key, value.toString()]))
              }
            };
          } else {
            return {
              tagName: 'meta',
              voidTag: true,
              attributes: Object.fromEntries(Object.entries(tag.attributes).map(([key, value]) => (typeof value === 'string' || typeof value === 'boolean') ? [key, value] : [key, value.toString()]))
            };
          }
        };
      ...
      const sourceScripts = injectOption === 'body' ? pluginBody : (htmlPluginData.plugin.options.scriptLoading === 'blocking' ? pluginBody : pluginHead);
      ...
      const copyAttributes = (tags, tagObjects) => {
        tags.forEach((tag, i) => {
          ...
          if (attributes) {
            ...
            Object.keys(attributes).forEach(attribute => {
              tagAttributes[attribute] = (typeof attributes[attribute] === 'string' || typeof attributes[attribute] === 'boolean') ? attributes[attribute] : attributes[attribute].toString();
            });
          }
        });
      };
    ...
    if (HtmlWebpackPlugin.getHooks || HtmlWebpackPlugin.getCompilationHooks) {
      ...
      hooks.beforeAssetTagGeneration.tapAsync('htmlRspackTagsPlugin', onBeforeHtmlGeneration);
      hooks.alterAssetTagGroups.tapAsync('htmlRspackTagsPlugin', onAlterAssetTagGroups);
    } else {
      const message = "Error running html-webpack-tags-plugin, are you sure you have html-webpack-plugin before it in your webpack config's plugins?";
      throw new Error(message);
    }
  ...
  compiler.hooks.compilation.tap('htmlRspackTagsPlugin', onCompilation);
};

HtmlRspackTagsPlugin.api = {
  IS,
  getValidatedOptions
};

module.exports = HtmlRspackTagsPlugin;
